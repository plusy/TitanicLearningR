...
FILE_RAW_DATA_BASE_DR3_TABLEAU <- file.path(DIR_MIDPUT, 'datareview3_tableua.csv')
FILE_RAW_DATA_BASE_DR3_RDS     <- file.path(DIR_MIDPUT, 'datareview3.RDS')
FILE_OUT_RF_MAGAN_P5_FINAL_CSV   <- file.path(DIR_MIDPUT, 'RFMagan_final_result.csv')

#  ------------------------------------------------------------------------
# Blog post: Exploring Survival on the Titanic
...
mice_mod <- mice(full[, !names(full) %in% c('DATA_TAG', 'PassengerId','Name','Ticket','Cabin','Family','Surname','Survived')],
                 method='rf') 
...
md.pattern(subset(full,select = -DataTag))


# write data for Tableau and Modeling --------------------------------------------------------------
write_csv(full, FILE_RAW_DATA_BASE_DR3_TABLEAU)
saveRDS(full, file = FILE_RAW_DATA_BASE_DR3_RDS)
...
prediction <- predict(rf_model, test,  type='prob')

evaluateROC(test$PassengerId, prediction[,2])

out <- data.frame(PassengerId=test$PassengerId,
                  Survived=ifelse(prediction[,1]<0.5, 1, 0), 
                  row.names=NULL)
...
write.csv(x=out,
          file=FILE_OUT_RF_MAGAN_P5_FINAL_CSV,
          row.names=FALSE,
          quote=FALSE)
